    ...
    private function processData(array $data) {
        ...
        foreach($data['formatters'] as $locale => $f) {
            ...
            if (is_array($f) && sizeof($f)) {
                foreach($f as $name => $rules) {
                    $this->formatters[$locale][$name] = new \NumberFormatter($locale, \NumberFormatter::PATTERN_RULEBASED, $rules['pattern']);
                    if (isset($rules['symbols']) && is_array($rules['symbols'])) {
                        foreach($rules['symbols'] as $attr => $val) {
                            $this->formatters[$locale][$name]->setSymbol(constant('NumberFormatter::' . strtoupper($attr)), $val);
                        }
                    }
                    
                    if (isset($rules['textAttributes']) && is_array($rules['textAttributes'])) {
                        foreach($rules['textAttributes'] as $attr => $val) {
                            $this->formatters[$locale][$name]->setTextAttribute(constant('NumberFormatter::' . strtoupper($attr)), $val);
                        }
                    }
                }
            }
