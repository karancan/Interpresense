...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-md-2">
            <a href="invoicesSubmitted.php?page=export&amp;start=<?= $filter_start_date->format('Y-m-d'); ?>&amp;end=<?= $filter_end_date->format('Y-m-d'); ?>&amp;approved_only=<?= $antiXSS->escape($_GET['approved_only'], $antiXSS::URL_PARAM); ?>" download target="_blank" class="btn btn-info btn-block admin-add-button"><i class="fa fa-file-excel-o"></i> Export (CSV format)</a>
        </div>
    </div>
                    ...
                    if (!empty($invoices)){
                        foreach($invoices as $i) {
                            echo "<tr data-invoice-id='{$antiXSS->escape($i['invoice_id'], $antiXSS::HTML_ATTR)}'
                                 ...
                                 "<td>" . ($i['is_approved'] ? '<span class="admin-invoice-approved-details" data-content="Approved on ' . $dateFmt->format($i['approved_on'], 'date_time') . ' by ' . $i['first_name'] . ' ' . $i['last_name'] . '">Yes <i class="fa fa-info-circle"></i></span>' : 'No') . "</td>" .
